...
METHODS = [
    ...
    {"name": "Correlation", "model": "StandardUNet", "type": "baseline", "ranking": "Correlation"},
    {"name": "Fisher",      "model": "StandardUNet", "type": "baseline", "ranking": "Fisher"},
    ...
    while jobs_queue or running_processes:
        ...
        for i, p_info in enumerate(running_processes):
            if p_info["process"].poll() is not None:
                ...
                print(f"({completed_count}/{total_jobs}) COMPLETED: {p_info['name']} with code {p_info['process'].returncode}")
        ...
        while len(running_processes) < MAX_CONCURRENT_WORKERS and jobs_queue:
            ...
            print(f"LAUNCHING ({len(running_processes) + 1}/{MAX_CONCURRENT_WORKERS}): {run_name} -> {log_file_path}")
            with open(log_file_path, "w") as out_log:
                p = subprocess.Popen(cmd, stdout=out_log, stderr=subprocess.STDOUT, env=env)
        ...
        print(f"--- {len(running_processes)} jobs running. {len(jobs_queue)} jobs in queue. Polling in {POLL_INTERVAL_SECONDS}s. ---")
